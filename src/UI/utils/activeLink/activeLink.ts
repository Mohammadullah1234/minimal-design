...
export default function isActiveLink(
  ...
  if (!currentPathname || !targetPath)
    return (console.warn("isActiveLink: pathname or itemPath is empty!"), !1);

  if (targetPath.startsWith("#") || isExternalLink(targetPath)) return !1;

  let path = removeLastSlash(currentPathname),
    target = removeLastSlash(removeParams(targetPath));

  return deep || hasParams(targetPath)
    ? path === target ||
